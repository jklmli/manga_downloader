...
@--threads@
Limits the number of chapter threads to the user specified value. Default value is 2.
...
* Each chapter download temporarly creates a temporary directory. There is a limit to the number of the temporary directories that a program 
  can use at once. If downloading a lot of chapters, the script because the OS refuses to create anymore temprary directories.
* The script can be a bandwidth hog when downloading a lot of manga chapters. 
