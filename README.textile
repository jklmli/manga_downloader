...
The destination download directory.  Defaults to current working directory.

@--overwrite@
Overwrites previous copies of downloaded chapters.

@-s, --subdirectory@
Creates a new subdirectory inside the current working directory using the name of the manga.
...
@-c, --convertFiles@
Converts the files that are downloaded to a Format/Size ratio acceptable to the device specified by --device
...
@--convertDirectory@
Converts the image files stored in the directory specified by the --inputDirectory parameter. Stores teh images
in the directory specified by the --outputDirectory Parameter

@--inputDirectory@
The directory containg the images to covert when --convertDirectory is specified. 
