...
* Program does not handle an empty node for LastChapterDownloaded. Ex: <LastChapterDownloaded/> 
* or <LastChapterDownloaded></LastChapterDownloaded>. If the node is empty, the scripts fails to 
* update it. Thus all the chapters are downloaded each time the script is run. 
* Workaround: Put a string value (Garbage) Ex: <LastChapterDownloaded>Garbage</LastChapterDownloaded>
* The next time the script runs it will update the field to the correct value. 
