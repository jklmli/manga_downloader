...
Currently supports mangavolume.com, cloudmanga.com, and otakuworks.com, with a total of over 3500 mangas.
...
@manga.py -n "Air Gear" -d "C:\Documents and Settings\admin\Documents\Manga\" -z@
On a Windows machine, downloads 'Air Gear' to C:\Documents and Settings\admin\Documents\Manga\ from chapter 1 to latest, using .zip compression

@./manga.py -n "Air Gear" -s 20 -d "~/Manga/" -o@
On a Linux/Unix machine, downloads 'Air Gear' to ~/Manga from chapter 20 to latest, using .cbz compression and overwriting previously downloaded chapters

h2. Bugs

* Program crash if user specifies chapter 0 as starting chapter and it doesn't exist.
* Program crash if there's a decimal chapter.
* Program crash if site is missing chapters in between.

h2. Issues

* (Animea, MangaReader) forces inefficient downloading of source code on every page due to unpredictable image filenames, forcing an overhead of about (100, 300) kB/page.
* Otakuworks, and to a rarer extent CloudManga and MangaVolume, sometimes has non-sequential chapter numbering (e.g. Vol 08 for chapters XXX-YYY).  Workaround will probably require user input or specified option.
* Cloudmanga downloader will download wrong manga (because of autocorrection) if given a more obscure manga - should be easy to fix, but the site search itself is down.
