...
Usage: manga.py [options] <manga name>

Options:
  |@--version@             				|	show program's version number and exit										|
  |@-h, --help@            				|	show this help message and exit											|
  |@--all@                 				|	Download all available chapters.										|
  |@-d DOWNLOAD_PATH, --directory=DOWNLOAD_PATH@	|	The destination download directory.  Defaults to current working directory.					|
  |@--overwrite@           				|	Overwrites previous copies of downloaded chapters.								|
  |@-s, --subdirectory@					|	Creates a new subdirectory inside the current working directory using the name of the manga.			|
  |@-x XMLFILE_PATH, --xml=XMLFILE_PATH@		|	Parses the .xml file and downloads all chapters newer than the last chapter downloaded for the listed mangas.	|
  |@-z, --zip@             				|	Downloads using .zip compression.  Omitting this option defaults to .cbz.					|

The script will offer a choice of 3 manga sites, it will default to the first if you press <enter>.
...
* Program does not handle if LastChapterDownloaded is an empty node in the XML Document. If the node is empty, the script fails to update it.
  Thus all the chapters are downloaded each time the script is run.
  Workaround: Put a string value (Garbage) as a placeholder. The next time the script runs it will update the field to the correct value.
...
* Chapter numbering on MangaReader doesn't pad, i.e. 1, 2, 3 as opposed to 001, 002, 003.
* Code hasn't been tested on Windows in a while.

h2. History

*v0.6.9* (12/26/10)
+ Heavy code cleanup (functionality remains the same, with some bugs fixed)
