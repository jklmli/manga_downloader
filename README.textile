...
Manga Downloader is a Windows/Linux python script that does what it says for mangavolume.com and manga.animea.net
Downloads into .cbz format, can optionally download into .zip instead.
Untested on Windows/Mac OS.
...
@-n {name of manga}@
Quotes are optional if there are no spaces or if escaping spaces with a backslash.
...
* Animea is broken - redownloading source code every page causes anti-leeching to kick in
...
Add support for mangareader.net - currently downloading is supporting, but the program is unable to parse the number of chapters it hosts, so it doesn't work right now.
