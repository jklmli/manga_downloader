...
class MangaReader(Aftv):
    class Chapter(Aftv.Chapter):
        VOLUME_AND_CHAPTER_FROM_URL_REGEX = re.compile('http://www.mangareader.net/((\d+)-(\d+)-(\d+)/)?[^/]+/(chapter-)?(?P<chapter>\d+)(\.html)?')
    ...
    class Series(Aftv.Series):
        CHAPTER_FROM_SOURCE_REGEX = re.compile('<a href="(?P<url>[^"]*)">[^<]*</a>[^<]*</td>')
        TEMPLATE_URL = 'http://www.mangareader.net{}'
