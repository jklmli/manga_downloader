...
class ES_MangaHere(SiteParserBase):
    ...
    def parseSite(self):
        ...
        print('Beginning ES_MangaHere check: %s' % self.manga)

        # jump straight to expected URL and test if manga removed
        url = 'http://es.mangahere.com/manga/%s/' % self.fixFormatting( self.manga )
        ...
        if (source is None or 'the page you have requested can' in source):
            ...
            url = 'http://es.mangahere.com/site/search?name=%s' % '+'.join(self.manga.split())
            if self.verbose_FLAG:
                print(url)

            try:
                source = getSourceCode(url, self.proxy)
                seriesResults = ES_MangaHere.re_getSeries.findall(source)
            ...
            # 0 results
                ...
                url = 'http://es.mangahere.com/manga/%s/' % keyword
                if self.verbose_FLAG:
                    print(url)
                source = getSourceCode(url, self.proxy)
        ...
        re_getChapters = re.compile('a.*?href="*?/manga/%s/(c[\d]+)/[^"]*?"' % keyword)
        self.chapters = re_getChapters.findall(source)
        #if not self.chapters:
            #if self.verbose_FLAG:
                #print ("Trying chapter only regex")
            #isChapterOnly = True
            #re_getChapters = re.compile('a.*?href="http://.*?mangahere.*?/manga/%s/(c[\d]+)/[^"]*?"' % keyword)
            #self.chapters = re_getChapters.findall(source)
        ...
        for i in range(0, len(self.chapters)):
            ...
            self.chapters[i] = ('http://es.mangahere.com/manga/%s/%s' % (keyword, self.chapters[i]), self.chapters[i], self.chapters[i])
            if (self.auto):
